...
public class TreeUntyper<IN,OUT> where IN :struct
{
    public static ISyntaxNode? Untype(ISyntaxNode<IN, OUT> node)
    {
        return node switch
        {
            ...
            EmptyNode<IN, OUT> empty => Untype(empty),
            SyntaxEpsilon<IN, OUT> epsilon => Untype(epsilon),
    ...
    private static EmptyNode? Untype(EmptyNode<IN, OUT> emptyNode)
    {
    ...
    private static SyntaxEpsilon? Untype(SyntaxEpsilon<IN, OUT> epsilon)
    {
